...
| Arquivo | Responsabilidade |
|---------|------------------|
| `ipc.ts` | **Fonte única** dos tipos do IPC e dos nomes de canais (`Channels`): `ChatEvent`, envelopes com `convId`, `PermissionRequest`/`Response` (com `questions?`/`answers?` do `AskUserQuestion`), `AskQuestion`/`AskQuestionOption`/`QuestionAnswer`, `BrowserFrame` (com `mime`), `BrowserState` (com `tabs[]` e `androidSize`), `BrowserInput`, `PickedElement`, `ImageAttachment`, **`FileAttachment`** (anexo não-imagem), `TokenUsage`, `StartAgentOptions`, **`AppConfig`/`DEFAULT_CONFIG`** (inclui `openai`, `remoteToken`, `remoteEnabled`), **`OpenAiConfig`/`OPENAI_VOICES`**, `StitchConfig`, **`OllamaConfig`** + **`OLLAMA_MODELS`/`OLLAMA_BASE_URL`/`isOllamaModel()`** (modelos Ollama Cloud + detecção/roteamento), **`modelSupportsVision()`** (Claude sempre, Ollama só o que estiver em `OLLAMA_VISION_MODELS` — hoje só Kimi K2.7 Code; o resto precisa do `vision_fallback_router`), **`CacheInfo`** (pasta de dados). **Download:** `DOWNLOADABLE_EXTS`/`isDownloadableFile`, `DOWNLOAD_MARKER`/`parseDownloads` (extrai `[[download:PATH]]` do texto). **Abas:** `TabKind` (incl. `stitch`), `TAB_KINDS`, `TabInfo`, `tabName()`. **Remoto:** `RemoteInfo`, `RemoteConversation` (com `model?`/`effort?`), `RemoteStatePayload` (com `models`/`modelEffort`/`effortLabels` — catálogo p/ os seletores do celular), `RemoteInboundMsg` (com `images?`), **`RemoteSetModelMsg`** (troca de modelo/esforço vinda do celular; canal `remoteSetModel`), `RemoteBuildProgressMsg`. **Esforço:** `EffortLevel` (`low`/`medium`/`high`/`xhigh`/`max`), `MODEL_EFFORT` (níveis suportados por modelo — Haiku só até `high`; Opus/Sonnet/Fable até `max`; Ollama não tem entrada, então o seletor some), `DEFAULT_EFFORT` (`high`); `StartAgentOptions.effort` repassa o nível ao SDK. `RateLimitStatus.updatedAt` (epoch ms do snapshot, para persistência/staleness). Canais novos: `configGet`/`Set`, **`cacheGetInfo`/`cacheChooseDir`**, **`kvGet`/`kvSet`** (store key→JSON), **`fileDownload`**, **`pathExists`**, **`openaiTranscribe`/`openaiTts`**, **`authStatus`/`authLogin`**, **`agentRefreshUsage`**, `browser*`, `remote*`. |
...
| `src/storage.ts` | Carrega/salva (assíncrono) conversas e estado da UI no **SQLite** da pasta de cache via `window.api.kvGet`/`kvSet` (chaves `agentcode.conversations.v1`, `agentcode.ui.v1`, **`agentcode.usage-limits.v1`**). **Migra** o valor antigo do `localStorage` na primeira leitura. Descarta o campo `images` ao persistir (o `draft` e o `error` da mensagem são persistidos junto da conversa). **`loadUsageLimits`/`saveUsageLimits`**: snapshot do uso da conta (5h/semana), para o badge aparecer já na abertura do app, antes do 1º evento ao vivo. Tolerante a erros. |
...
| `src/vad.ts` | **VAD local** (detecção de voz) para o ditado, sem biblioteca externa — funções puras e testáveis: `frameRms` (RMS do waveform), `newVadState`/`vadStep` (marca fala, fecha a fala na pausa `VAD_SILENCE_HOLD_MS` ou no teto `VAD_MAX_SEG_MS`; limiar `VAD_SPEECH_RMS`), `tryArmedTrigger` (transição "armado" → gravando: enquanto não detecta fala retorna `null` — o `Composer` não grava nada nesse meio tempo; no quadro que cruza o limiar, devolve um `VadState` já com `hadSpeech: true`, então a gravação nasce exatamente na fala, sem silêncio de antes embutido no áudio). Testes em `src/vad.test.ts`. |
...
const CONTEXT_LIMITS: Record<string, number>                  // janela de contexto REAL por modelo (Opus/Sonnet/Fable=1M, Haiku=200K; Ollama: Qwen3-Coder=256K, gpt-oss=128K, DeepSeek V4 Pro=1M, GLM-5.2=1M, Kimi K2.7=256K — verificados contra os specs de cada modelo)
...
function modelSupportsVision(model?: string): boolean          // Claude sempre true; Ollama só true p/ modelos em OLLAMA_VISION_MODELS (hoje só kimi-k2.7-code:cloud) — o resto passa pelo vision_fallback_router
