...
| `claudeCli.ts` | `claudeCliPath()` resolve o **binário do CLI nativo do Claude Code** que o Agent SDK distribui como dependência opcional por plataforma (`@anthropic-ai/claude-agent-sdk-<plat>-<arch>`, `-musl` no Linux) via `require.resolve`; cacheia e lança se não achar. **Esse binário é diferente** da instalação global do `claude` (a que `claude update` atualiza no sistema) — vive dentro do `node_modules`, e sua versão é fixada pela versão do `@anthropic-ai/claude-agent-sdk` no `package.json`. Rodar `claude update` no sistema **não** atualiza o CLI que o app usa para o login (ver `login.ts`); pra isso, atualize a dependência `@anthropic-ai/claude-agent-sdk` no projeto. |
