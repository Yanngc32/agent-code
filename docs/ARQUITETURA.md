...
- **Contexto de entrada** = `ctx` (`tokens.context`) = o que está sendo **enviado** ao modelo naquela requisição (a janela que ele recebe). É o numerador da **barra de limite de contexto** (`ContextBar`); o denominador é o limite do modelo (`contextLimitFor(model)` — `CONTEXT_LIMITS` em `shared/ipc.ts`: Opus/Sonnet/Fable 1M, Haiku 200K — valores Anthropic **autoritativos**; Ollama Cloud com a janela **nativa real** de cada modelo — Qwen3-Coder 256K, gpt-oss 128K, **DeepSeek V4 Pro e GLM-5.2 1M** (verificados contra a documentação de cada modelo — antes estavam sub-relatados em 128K/200K, o que fazia a barra parecer cheia bem antes do limite de verdade), Kimi K2.7 Code 256K; fallback `DEFAULT_CONTEXT_LIMIT`). A barra mostra `usado / limite` e a % de preenchimento. Manter o mapa em sinc ao adicionar modelo ao seletor — limite errado = barra errada.
...
**Silêncio nunca é gravado (não só nunca enviado)** — enquanto **"armado"** (`segRef.current === null`: logo que o mic abre, antes da 1ª palavra, ou no intervalo entre duas falas), o `MediaRecorder` **não está rodando**. `tryArmedTrigger(rms, now)` (em `vad.ts`) roda a cada frame nesse estado: enquanto o quadro é silêncio devolve `null` (fica armado, nada é capturado); no instante em que o RMS cruza `VAD_SPEECH_RMS`, devolve um `VadState` **já com `hadSpeech: true`** e o `Composer` chama `startSegment(stream, vad)` **naquele exato quadro** — a gravação nasce na fala, então não existe silêncio de antes pra cortar depois (era exatamente aí que o modelo inventava palavras: um silêncio longo antes de falar ficava embutido no início do áudio enviado). Ao fechar a frase (`endUtterance`), o segmento é parado e transcrito, e o app **volta a ficar armado** — não abre o próximo segmento de cara — então o intervalo de silêncio entre duas falas também nunca é gravado.

O `runMeter` (no `requestAnimationFrame`) lê o waveform **uma vez por frame** (`getByteTimeDomainData` → `frameRms`) e usa o mesmo RMS para: (a) se armado, checar `tryArmedTrigger`; se já gravando, avançar `vadStep` e chamar `endUtterance()` na pausa (ou no teto de segurança `VAD_MAX_SEG_MS`); (b) alimentar a **forma de onda da gravação** (ver abaixo — a forma de onda/cronômetro continuam rodando o tempo todo, mesmo em silêncio/armado, só o que é **enviado à API** é que muda). `onstop` só transcreve se `vad.hadSpeech` (agora sempre verdadeiro na prática — vira uma rede de segurança). O texto novo é anexado a `baseTextRef` + `transcriptRef`. Uma **setinha** (`mic-caret`) ao lado do mic abre o menu de escolha do microfone (`enumerateDevices` → `audioinput`; o `deviceId` é persistido em `localStorage` na chave `agentcode.micId`, com fallback ao padrão se o device sumir — `OverconstrainedError`). A permissão de mic é liberada no Electron (ver [Modelo de processos](#modelo-de-processos-e-segurança)).
...
- **Quem suporta visão de verdade** (`modelSupportsVision(model)` em `shared/ipc.ts`): todo modelo Claude, sempre; no Ollama, só o que estiver em `OLLAMA_VISION_MODELS` (hoje só `kimi-k2.7-code:cloud` — o Kimi K2.7 Code tem um encoder de visão próprio, o MoonViT, e aceita imagem nativamente). Os outros cinco (Qwen3-Coder, gpt-oss 120B/20B, DeepSeek V4 Pro, GLM-5.2) são texto-only e passam pelo relay.
